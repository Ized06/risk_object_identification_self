...
class GCN_intention(nn.Module):
    def __init__(self, inputs, time_steps=90, pretrained= True , partialConv = False, fusion = 'avg', args=None):
        super(GCN_intention, self).__init__()
        ...
        # build backbones
        self.resnet = ResNet(structure= [3, 4, 6, 3], bottleneck= True)
        self.load_resnet_model(self.resnet)
        self.trn = TopoTRN(args)
        trn_checkpoint = '/home/zxiao/project/TRN.pytorch/work_dir/4_way_stop_intention_debug_intention/inputs-camera-epoch-56.pth'
        checkpoint = torch.load(trn_checkpoint, map_location=torch.device('cpu'))
        self.trn.load_state_dict(checkpoint['model_state_dict'])
        ...
        self.vel_classifier_v2= nn.Sequential(
            nn.Linear(self.hidden_size*2+self.trn.hidden_size, 100),
            nn.ReLU(inplace=True),
            nn.Linear(100, 50),
            nn.ReLU(inplace=True),
            nn.Linear(50, 10),
    ...
    # load pre-trained model
    def load_mapillary_weights(self,weight_path):

        snapshot = torch.load(weight_path)
        body = snapshot['state_dict']
        state_dict = body['body']
        
        return state_dict
    def load_resnet_model(self,model):
        pretrain = 'mapillary'
        if pretrain=='imagenet':
            state_dict = torch.load('/home/zxiao/project/feature_extractor/zxiao_extract_feats/resnet50-19c8e357.pth')
        elif pretrain=='mapillary':
            state_dict = self.load_mapillary_weights('/home/zxiao/project/feature_extractor/zxiao_extract_feats/seamseg_r50_vistas.tar')
        ...
        print('Loaded model states for model Resnet-50 pretrained on %s' % pretrain)
    def loadmodel(self,filepath):
        state_dict = model_zoo.load_url(filepath)
        if self.partialConv:
            #process pretrained model dict for partialConv inceptionv2
    ...
    def forward(self, camera_inputs, trackers, device, dist_mask , mask = None):
        ...
        # CNN
        trn_camera_feature = self.resnet.features(camera_inputs.view(-1,c,w,h))
        trn_camera_feature_shape = trn_camera_feature.shape
        trn_camera_feature = trn_camera_feature.view(-1,t,trn_camera_feature_shape[-3],trn_camera_feature_shape[-2],trn_camera_feature_shape[-1])
        trn_sensor_inputs = torch.zeros(batch_size,t,20).to(device)
        intention_feature,waypoint_enc_scores, intention_enc_scores,waypoint_dec_scores,reconstructed_dec_features = self.trn(trn_camera_feature,trn_sensor_inputs)
        if self.partialConv:
            camera_inputs = self.backbone.features(camera_inputs.view(-1,c,w,h), mask.view(-1,c,w,h)) # camera_inputs :(bs,t,c,w,h)
        else:
            camera_inputs = self.backbone.features(camera_inputs.view(-1,c,w,h)) # camera_inputs :(bs,t,c,w,h)
        ...
        updated_feature = self.message_passing(hx , trackers, dist_mask) #BxH
        object_feature = self.drop(updated_feature)
        feature = torch.cat((intention_feature, object_feature),1)
        # vel = self.vel_classifier(self.drop(updated_feature))
        vel = self.vel_classifier_v2(feature)
        logit_vel_stack.append(vel)
        logit_vel_stack = torch.stack(logit_vel_stack).view(-1, 2)
